    ...
    <div className="fixed inset-0 z-40 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full overflow-hidden border border-slate-200">
        ...
        <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-slate-800">Importer des données IA</h2>
          <div className="flex gap-2">
            <span className={`w-2 h-2 rounded-full ${step >= 1 ? 'bg-blue-600' : 'bg-slate-300'}`}></span>
            <span className={`w-2 h-2 rounded-full ${step >= 2 ? 'bg-blue-600' : 'bg-slate-300'}`}></span>
            <span className={`w-2 h-2 rounded-full ${step >= 3 ? 'bg-blue-600' : 'bg-slate-300'}`}></span>
          </div>
        </div>
        ...
        <div className="p-8 min-h-[300px] flex flex-col justify-center">
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors cursor-pointer relative">
                ...
                <h3 className="font-medium text-slate-900">Glissez-déposez votre export (ChatGPT, Claude...)</h3>
                <p className="text-sm text-slate-500 mt-2">ou cliquez pour parcourir (.json, .zip)</p>
                {fileName && (
                  <div className="mt-4 px-3 py-1 bg-green-100 text-green-700 text-sm rounded-full font-medium flex items-center gap-2">
            ...
            <div className="flex flex-col items-center text-center space-y-6">
              {isProcessing ? (
                <>
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-slate-800">Chiffrement en cours...</h3>
                    <p className="text-slate-500 mt-2">Nous sécurisons vos données avec une clé AES-256 locale.</p>
                  </div>
                </>
              ) : (
                <div className="space-y-4">
                  ...
                  <h3 className="text-xl font-semibold text-slate-800">Prêt à importer</h3>
          ...
          {step === 3 && (
            <div className="flex flex-col items-center text-center space-y-6 animate-in fade-in zoom-in duration-300">
              ...
              <div>
                <h3 className="text-2xl font-bold text-slate-800">Importation réussie !</h3>
        ...
        <div className="bg-slate-50 px-6 py-4 border-t border-slate-100 flex justify-between">
          {step < 3 ? (
            <>
              <button onClick={onCancel} className="px-4 py-2 text-slate-600 hover:text-slate-900 font-medium">
                Annuler
              </button>
              {step === 1 ? (
                <button 
